...
export const assertCateRow = (row: any, isJson: boolean) => {
  ...
  assert(
    isNotEmpty(category_code) && isNotEmpty(category_name) && isNotEmpty(created_date) && isNotEmpty(last_updated_date),
...
export const assertItemRow = (row: any, isJson: boolean) => {
  ...
  assert(
    isNotEmpty(item_code) &&
      isNotEmpty(category_code) &&
      isNotEmpty(title) &&
      isNotEmpty(image_url) &&
      isNotEmpty(created_date) &&
      isNotEmpty(last_updated_date),
    getErrorMsg(ERROR_MESSAGE.NOT_FOUND_REQUIRED_COL_ERROR, row),
  );
